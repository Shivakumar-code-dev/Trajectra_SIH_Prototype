...
import { Cpu, Database, Server, Lock, Code, Radio, Camera, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
export const SystemStatusPage: React.FC = () => {
  ...
  const pipelineSteps = [
    ...
    { title: 'YOLOv8 VEHICLE DETECTION', desc: 'Object Classification (Car, Bus, Truck, Bike)', icon: Cpu, color: 'text-emerald-400', border: 'border-emerald-500/40' },
    ...
    { title: 'CROSS-CAMERA MATCHING', desc: 'Spatial & Temporal Correlation across Camera Nodes', icon: Server, color: 'text-rose-400', border: 'border-rose-500/40' },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#1b2a4a] pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 font-sans mt-0.5">
            Full-stack AI Engine pipeline design, database schema, Flask REST APIs, and privacy-by-design policy
          </p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-[#1b2a4a] bg-[#0d1527] rounded-xl px-4">
        <button
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pipelineSteps.map((step, idx) => {
                ...
                return (
                  <div key={idx} className={`p-4 rounded-xl bg-[#0d1527] border ${step.border} shadow-lg flex items-center space-x-3`}>
                    <div className={`p-2.5 rounded-lg bg-slate-950 ${step.color}`}>
        ...
        {activeTab === 'database' && (
          <div className="space-y-6">
            {databaseSchemas.map((schema) => (
              <div key={schema.tableName} className="rounded-2xl bg-[#0d1527] border border-[#1b2a4a] p-4 space-y-3">
        ...
        {activeTab === 'api' && (
          <div className="space-y-4">
            {mockApiEndpoints.map((endpoint, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-[#0d1527] border border-[#1b2a4a] space-y-2 font-mono text-xs">
        ...
        {activeTab === 'privacy' && (
          <div className="space-y-4 text-xs font-sans text-slate-300 leading-relaxed">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              <div className="p-4 rounded-xl bg-[#0d1527] border border-[#1b2a4a] space-y-2">
